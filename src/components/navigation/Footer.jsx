...
const Footer = () => {
  return (
    <div className='border-t-2 border-verdeneon w-full py-20 h-auto  bg-gradient-to-r from-pretoneon via-pretoneon to-pretoneon border-b-black grid grid-cols-1 '>
    <div className="  grid grid-cols-1 md:grid-cols-2 lgl:grid-cols-4 gap-8 ">
      <div className="w-full h-full flex items-center flex-col gap-8">
        ...
        <div className="grid grid-cols-4  md:space-x-0 md:flex md:justify-center lg:justify-start ">
                    {navigation.social.map((item) => (
                      <a key={item.name} href={item.href} aria-label="add to slack"  className="p-4   rounded-full duration-300 hover:border-verdeneon hover:shadow-xl hover:shadow-verdeneon ">
                        {/* <span className="sr-only">{item.name}</span> */}
                        <item.icon className={`h-6 w-6 ${item.color} hover:animate-bounce  " aria-hidden="true" alt="slack logo" loading="lazy`} />
                      </a>
                    ))}
        </div>
      </div>
      ...
      <div className="w-full h-full">
        ...
        <ul className="flex flex-col gap-4 text-white font-titleFont font-medium overflow-hidden py-6">
          <li>
            <span className="w-full text-lg relative hover:text-rosapink duration-300 group cursor-pointer">
              Documentation
              <span className="w-full h-[1px] bg-designColor inline-flex absolute left-0 -bottom-1 -translate-x-[100%] group-hover:translate-x-0 transition-transform duration-300"></span>
            </span>
          </li>
